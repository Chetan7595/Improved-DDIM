...
class ResnetBlock(nn.Module):
    ...
    def forward(self, x, time_emb=None):
        ...
        if time_emb is not None:
            scale_shift = self.mlp(time_emb)[..., None, None]
...
class Attention(nn.Module):
    ...
    def forward(self, x):
        ...
        similarity = torch.einsum('b h n f, b h m f -> b h n m', q, k)
        attention_score = torch.softmax(similarity * self.scale, dim=-1)
...
class Unet(nn.Module):
    def __init__(self, dim, image_size, dim_multiply=(1, 2, 4, 8), channel=3, attn_heads=4, attn_head_dim=32,
                 full_attn=(False, False, False, True), resnet_group_norm=8, base_step_size=0.1, device='cuda'):
        super().__init__()
    ...
    def compute_complexity(self, x):
        ...
        complexity = torch.sqrt(grad_x ** 2 + grad_y ** 2).mean(dim=[2, 3])
        return complexity / (complexity.norm(p=2, dim=-1, keepdim=True) + 1e-5)
    ...
    def forward(self, x, time):
        ...
        adaptive_step = self.base_step_size / (complexity + 1e-5)
        concat = list()

        for block1, block2, attn, downsample in self.down_path:
            x = block1(x, t * adaptive_step)
            concat.append(x)
            x = block2(x, t * adaptive_step)
            x = attn(x) + x
            concat.append(x)
            x = downsample(x)

        x = self.mid_resnet_block1(x, t * adaptive_step)
        x = self.mid_attention(x) + x
        x = self.mid_resnet_block2(x, t * adaptive_step)
        ...
        x = self.final_resnet_block(x, t * adaptive_step)
